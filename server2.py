...
app = Flask(__name__, template_folder=".././reactExperiments")
...
def load_checkpoint(model,filename='./hsbc-7.pth.tar'):
...
def send_css(path):
   return send_from_directory('.././reactExperiments/css/', path)
...
def sendjs(path):
   print(path[:-4])
   return send_from_directory('.././reactExperiments/js/', path)


@app.route('/imgs/<path:path>')
def sendImg(path):
   print(path[:-4])
   return send_from_directory('.././reactExperiments/imgs/', path)
